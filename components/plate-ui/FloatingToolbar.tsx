...
import { useFloatingToolbar, useFloatingToolbarState } from '@udecode/plate-floating';
	...
	({ state, children, ...props }, componentRef) => {
		...
		return (
			<PortalBody>
				<Toolbar
					ref={ref}
					className={cn('absolute z-50 whitespace-nowrap border bg-popover px-1 opacity-100 shadow-md')}
					{...rootProps}
					{...props}
				>
					{children}
